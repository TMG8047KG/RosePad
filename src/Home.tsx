...
function App() {
  ...
  const handleCreateProject = async (name: string) => {
    const raw = await readTextFile(settingsFile, osType() !== "android" ? { baseDir: BaseDirectory.AppLocalData} : { baseDir: BaseDirectory.AppConfig })
    const data = JSON.parse(raw)
    let dir = data.projectPath;
    console.log(dir);
    if((osType() !== "android" || "ios") && dir == "null" || !dir){
      dir = await selectDir()
  ...
  return (
    <main>
      ...
      {osType() === "linux" || "windows" || "macos" ? <NavBar/> : ""}
