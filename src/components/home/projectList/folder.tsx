...
export function Folder({ id, type, name, projectIds, projectMap, onChanged, color }: { id:string; type:FolderType; name:string; projectIds:string[]; projectMap:Record<string,Project>; onChanged:()=>void; color?:string|null }) {
  ...
  const projectOptions = Menu.new({
    // Ensure menu id is unique across instances
    id: `folderOptions_${type}_${id}`,
    items: [
      { id: `${type}:${id}:rename`, text: "Rename", action: () => { setIsRenameOpen(true) }},
      // Open color modal with the folder's current color preselected
  ...
  const bg = color ? toAlpha(color, 0.50) : undefined
  ...
  return (
    <>
      ...
      <div className={style.container} data-type={type}>
        <div className={style.head} style={bg ? { background: `${bg}` } : undefined}>
          <h3>{name}</h3>
          <div className={style.right}>
            ...
            <button className={style.index} onClick={toggleList} aria-expanded={!collapsed}>
              <svg style={collapsed ? undefined : { transform: 'rotate(-90deg)' }} className="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
